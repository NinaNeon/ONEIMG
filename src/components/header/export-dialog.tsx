...
export function ExportImageDialog({
      ...
      if (previewRef.current) {
        ...
        try {
          ...
          if (previewRef.current.containerRef.current) {
            // ★ 書頁排版模式 (page-style / page-frame): 跳過整體長圖, 直接每張卡片各自一張 PNG
            const isPagedTemplate = templateName === 'page-style' || templateName === 'page-frame'
            if (!isPagedTemplate) {
            ...
            // 导出每个顶层 PreviewItem (page-style 下每個 key 是 "{contentId}__page{N}")
            const itemRefs = previewRef.current.itemRefs.current
            if (itemRefs) {
              for (const [id, ref] of Object.entries(itemRefs)) {
                if (ref) {
                  // 解析 key: 一般情況是純數字 contentId,
                  // page-style 分頁時是 "5__page0" 這種格式
                  const idStr = String(id)
                  let contentId: number
                  let pageSuffix = ''
                  if (idStr.includes('__page')) {
                    const [cidPart, pagePart] = idStr.split('__page')
                    contentId = Number(cidPart)
                    // 從 0 起跳, 檔名顯示為 1 起跳, 補零至 2 位數方便排序
                    const pageNum = Number(pagePart) + 1
                    pageSuffix = `_p${String(pageNum).padStart(2, '0')}`
                  } else {
                    contentId = Number(idStr)
                  }
                  const title = removeHtmlTags(dataMap.get(contentId)) || 'page'
                  const filename = `${index}_${title}${pageSuffix}.png`
                  const cardPreviewBlobObject = await exportImage(ref, filename, exportOption)
  ...
  }, [previewRef, scale, setIsExporting, isExporting, templateName])
